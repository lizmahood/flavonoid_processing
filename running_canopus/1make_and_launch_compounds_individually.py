...
def launch_sirius(mgf, odir, mstype):
    ...
    '''
    command = f'sirius -o {odir} -i {mgf} formula -c 3 -p {mstype} '\
...
def make_mgf_for_compound(sirpardir, mgfinpdir, mgfstr, alid, ct):
    '''
...
def main(mgfinp, sirpardir, mgfpardir, alifil, mstype):
        ...
        while inlin:
            if inlin.startswith('BEGIN'):
                ...
                if scans in alid.keys():
                    nam, sirdir = make_mgf_for_compound(sirpardir, mgfpardir, ostring, alid, ct)
                    launch_sirius(nam, sirdir, mstype)
                    ct += 1
